...
type GeminiWebAction = (typeof GEMINI_WEB_ACTIONS)[number]
type Service = Awaited<ReturnType<typeof ensureServiceInitialized>>

export function registerGeminiWebCommands(cli: CAC): void {
  cli
    .command(
      'geminiWeb <action> [...args]',
      `Gemini Web provider utilities (${GEMINI_WEB_ACTIONS.join('|')})`
    )
    .option('--reason <text>', 'Reason for set-status')
    .option('--model <model>', 'Gemini Web model for chat smoke test', {
    ...
    .action(async (action: string, args: string[] = [], options: GeminiWebOptions) => {
      if (!GEMINI_WEB_ACTIONS.includes(action as GeminiWebAction)) {
        throw new CliError(
          `Unknown Gemini Web action: ${action}. Expected one of ${GEMINI_WEB_ACTIONS.join(', ')}.`,
          { code: ExitCode.UsageError, errorCode: 'UNKNOWN_ACTION' }
        )
      }
      const service = await ensureServiceInitialized()
      switch (action as GeminiWebAction) {
        case 'list':
        ...
        case 'chat':
          return runChat(service, {
            model: String(options.model || 'gemini-3.5-flash'),
...
async function runList(service: Service): Promise<void> {
  ...
  const provider = status.providers.find((item) => item.providerType === 'geminiWeb') as any
  const accounts: any[] = provider?.accounts ?? []
  emitSuccess(accounts, () => {
    if (isJsonMode()) return
    if (!accounts.length) {
      process.stdout.write(`${colors.dim('(no Gemini Web accounts)')}\n`)
      return
    }
    printTable(accounts, [
...
async function runImportJson(service: Service, source: string | undefined): Promise<void> {
  ...
  if (!text) {
    throw new CliError('No Gemini Web cookie JSON content provided.', {
      code: ExitCode.UsageError,
      errorCode: 'MISSING_JSON'
    })
  }
  const result = await service.importGeminiWebAuthJson(text)
...
async function runTest(service: Service, id: string): Promise<void> {
  const result = await service.testGeminiWebAccount(id)
  await notifyDaemonReload().catch(() => false)
  emitSuccess(result, () => {
    process.stdout.write(
...
async function runInfo(service: Service, id: string): Promise<void> {
  emitSuccess(await service.getGeminiWebAccountInfo(id))
}

async function runRefreshModels(service: Service, id: string): Promise<void> {
  const result = await service.refreshGeminiWebAccountModels(id)
  await notifyDaemonReload().catch(() => false)
  emitSuccess(result, () =>
...
async function runToggle(service: Service, id: string, enabled: boolean): Promise<void> {
  await service.toggleGeminiWebAccount(id, enabled)
  await notifyDaemonReload().catch(() => false)
...
async function runRemove(service: Service, id: string): Promise<void> {
  await service.removeGeminiWebAccount(id)
  await notifyDaemonReload().catch(() => false)
...
async function runReset(service: Service, id: string): Promise<void> {
  await service.resetGeminiWebAccount(id)
  await notifyDaemonReload().catch(() => false)
...
async function runSetStatus(
  ...
  await service.setGeminiWebAccountStatus(id, status as any, reason)
  await notifyDaemonReload().catch(() => false)
...
async function runChat(
  ...
  try {
    ...
    if (!response.ok || payload?.error) {
      throw new CliError(
        `Gemini Web chat failed: HTTP ${response.status} ${JSON.stringify(payload).slice(0, 800)}`,
        { code: ExitCode.GeneralError, errorCode: 'GEMINI_WEB_CHAT_FAILED' }
      )
    }
    emitSuccess(payload, () => {
