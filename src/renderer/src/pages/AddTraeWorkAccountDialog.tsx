...
export function AddTraeWorkAccountDialog({
  open,
  onOpenChange,
  onImported
}: {
  ...
  async function handleRefreshToken() {
    ...
    setTokenMsg(null)
    try {
      await window.api.gateway.addTraeWorkRefreshToken(refreshToken)
  ...
  async function handleJwtToken() {
    ...
    setTokenMsg(null)
    try {
      await window.api.gateway.addTraeWorkJwtToken(jwtToken)
      setTokenMsg({ ok: true, text: t('addAccount.tokenValid') })
      setJwtToken('')
      onImported()
      onOpenChange(false)
  ...
  async function handleJson() {
    ...
    setJsonResult(null)
    try {
      const r = await window.api.gateway.importTraeWorkJson(jsonText)
  ...
  async function handleScan() {
    ...
    try {
      const r = await window.api.gateway.scanTraeWorkAccounts()
      setScanResult(r)
  ...
  async function handleImportSelected() {
    ...
    setDiscoverLoading(true)
    try {
      await window.api.gateway.importScannedTraeWorkAccounts([...selectedIds])
      setScanResult(null)
      setSelectedIds(new Set())
      onImported()
      onOpenChange(false)
  ...
  return (
    <Modal open={open} onOpenChange={onOpenChange} title={t('addAccount.traeWorkTitle')}>
      <TabGroup
        value={tab}
        ...
        items={[
          {
            value: 'refresh',
            label: t('addAccount.tabs.refreshToken'),
            content: (
              <div className="space-y-3">
                <p className="text-[12px] text-fog">{t('addAccount.traeWorkRefreshHint')}</p>
                <textarea
                  value={refreshToken}
                  onChange={(e) => setRefreshToken(e.target.value)}
                  placeholder={t('addAccount.traeWorkRefreshPlaceholder')}
                ...
                <div className="flex items-center gap-3">
                  <Button
                    ...
                    disabled={tokenLoading || !refreshToken.trim()}
                  >
          ...
          },
          {
            ...
            content: (
              <div className="space-y-3">
                <p className="text-[12px] text-fog">{t('addAccount.traeWorkJwtHint')}</p>
                <textarea
                  value={jwtToken}
                  onChange={(e) => setJwtToken(e.target.value)}
                  placeholder={t('addAccount.traeWorkJwtPlaceholder')}
                  className="input-base font-mono text-[12px] min-h-20 resize-y w-full"
                />
                <div className="flex items-center gap-3">
                  <Button
                    variant="primary"
                    onClick={handleJwtToken}
                    disabled={tokenLoading || !jwtToken.trim()}
                  >
                    {tokenLoading ? t('addAccount.tokenValidating') : t('common.add')}
                  </Button>
                  {tokenMsg && (
          ...
          },
          {
            ...
            content: (
              <div className="space-y-2">
                <p className="text-[12px] text-fog">{t('addAccount.traeWorkDiscoverDesc')}</p>
                {!scanResult && (
                        ...
                        >
                          ...
                          <div className="flex-1 min-w-0">
                            ...
                            <p className="text-[12px] text-fog font-mono truncate">
                              {c.email || c.countryCode || c.sourceType || c.id}
                            </p>
                          </div>
                          <span className="tag text-[12px] !px-1 !py-0 shrink-0">
                            {c.existing ? t('gateway.exists') : c.sourceType || 'traework'}
                          </span>
                        </label>
                      ))}
                    </div>
          ...
          },
          {
            ...
            content: (
              <div className="space-y-2">
                <p className="text-[12px] text-fog">{t('addAccount.traeWorkJsonHint')}</p>
                <textarea
                  className="w-full min-h-[120px] text-[12px] font-mono bg-pitch rounded-[var(--radius-md)] p-2 text-storm"
                  value={jsonText}
                  onChange={(e) => setJsonText(e.target.value)}
                  placeholder={t('addAccount.traeWorkJsonPlaceholder')}
                />
                <div className="flex items-center gap-2">
                  <Button
                    variant="primary"
