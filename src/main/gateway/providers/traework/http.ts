...
export async function traeWorkFetch(
  url: string,
  init: RequestInit,
  settings?: Pick<TraeWorkProviderSettings, 'vpnProxyUrl'>
): Promise<Response> {
  ...
  try {
    ...
    throw new Error(`TraeWork proxy setup failed for ${proxyUrl}: ${toErrorMessage(error)}`)
  }
}
