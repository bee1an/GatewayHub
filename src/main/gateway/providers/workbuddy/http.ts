...
export async function workBuddyFetch(
  url: string,
  init: RequestInit,
  settings?: Pick<WorkBuddyProviderSettings, 'vpnProxyUrl'>
): Promise<Response> {
  ...
  try {
    ...
    throw new Error(`WorkBuddy proxy setup failed for ${proxyUrl}: ${toErrorMessage(error)}`)
  }
}
