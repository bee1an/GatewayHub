...
describe('nvidia/provider request race', () => {
  afterEach(() => {
    vi.unstubAllGlobals()
  })
  ...
  it('uses the fastest complete successful JSON response and aborts pending losers', async () => {
    ...
    await provider.initialize([
      { id: 'slow', enabled: true, apiKey: 'nvapi-slow' },
      { id: 'fast', enabled: true, apiKey: 'nvapi-fast' },
      { id: 'rate', enabled: true, apiKey: 'nvapi-rate' }
    ])
    let slowAborted = false
    const fetchMock = vi.fn(async (_url: string, init?: RequestInit) => {
      const auth = String((init?.headers as any)?.Authorization || '')
      if (auth.endsWith('nvapi-rate')) {
        await delay(5)
        return Response.json({ error: 'rate limited' }, { status: 429 })
      }
      if (auth.endsWith('nvapi-fast')) {
        await delay(25)
        return Response.json({
          id: 'fast-winner',
          choices: [],
