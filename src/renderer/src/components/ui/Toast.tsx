...
const TYPE_CONFIG: Record<ToastType, { color: string; accent: string; progress: string; iconPath: React.JSX.Element }> = {
  success: {
    color: 'text-emerald',
    accent: 'bg-emerald',
...
export function ToastProvider({ children }: { children: ReactNode }): React.JSX.Element {
  ...
  const dismiss = useCallback((id: number) => {
    clearTimer(id)
    setItems((prev) => prev.map((t) => t.id === id && t.phase !== 'exiting' ? { ...t, phase: 'exiting' } : t))
  ...
  return (
    <ToastContext.Provider value={{ toast }}>
      {children}
      <div className="fixed top-4 right-4 flex flex-col gap-2.5 pointer-events-none" style={{ zIndex: 9999 }}>
        {items.map((item) => {
          ...
          return (
            ...
            >
              ...
              <div className="flex items-center gap-2.5">
                <svg width="14" height="14" viewBox="0 0 14 14" fill="none" className={`shrink-0 ${cfg.color}`}>
                  {cfg.iconPath}
                </svg>
