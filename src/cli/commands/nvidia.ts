...
import { NVIDIA_DEFAULT_SMOKE_MODEL } from '../../main/gateway/providers/nvidia/constants'

const NVIDIA_ACTIONS = [
  'list',
  'import-key',
  'import-json',
  'test',
...
type NvidiaAction = (typeof NVIDIA_ACTIONS)[number]
type Service = Awaited<ReturnType<typeof ensureServiceInitialized>>

export function registerNvidiaCommands(cli: CAC): void {
  cli
    .command('nvidia <action> [...args]', `NVIDIA provider utilities (${NVIDIA_ACTIONS.join('|')})`)
    .option('--reason <text>', 'Reason for set-status')
    .option('--model <model>', 'NVIDIA model id for chat smoke test', {
      default: NVIDIA_DEFAULT_SMOKE_MODEL
    })
    ...
    .action(async (action: string, args: string[] = [], options: NvidiaOptions) => {
      if (!NVIDIA_ACTIONS.includes(action as NvidiaAction)) {
        throw new CliError(
          `Unknown NVIDIA action: ${action}. Expected one of ${NVIDIA_ACTIONS.join(', ')}.`,
          { code: ExitCode.UsageError, errorCode: 'UNKNOWN_ACTION' }
        )
      }
      const service = await ensureServiceInitialized()
      switch (action as NvidiaAction) {
        case 'list':
        ...
        case 'chat':
          return runChat(service, {
            model: String(options.model || NVIDIA_DEFAULT_SMOKE_MODEL),
...
async function runList(service: Service): Promise<void> {
  ...
  const provider = status.providers.find((item) => item.providerType === 'nvidia') as any
  const accounts: any[] = provider?.accounts ?? []
  emitSuccess(accounts, () => {
    if (isJsonMode()) return
    if (!accounts.length) {
      process.stdout.write(`${colors.dim('(no NVIDIA keys)')}\n`)
      return
    }
    printTable(accounts, [
...
async function runImportKey(service: Service, text: string | undefined): Promise<void> {
  ...
  if (!value) {
    throw new CliError('Provide NVIDIA API key as argument or via stdin.', {
      code: ExitCode.UsageError,
      errorCode: 'MISSING_KEY'
    })
  }
  const result = await service.addNvidiaApiKey(value.trim())
  await notifyDaemonReload().catch(() => false)
  emitSuccess(result, () => process.stdout.write(`${colors.green('imported')} NVIDIA key\n`))
}
...
async function runImportJson(service: Service, source: string | undefined): Promise<void> {
  ...
  if (!text) {
    throw new CliError('No NVIDIA JSON content provided.', {
      code: ExitCode.UsageError,
      errorCode: 'MISSING_JSON'
    })
  }
  const result = await service.importNvidiaAuthJson(text)
  await notifyDaemonReload().catch(() => false)
  emitSuccess(result, () =>
...
async function runTest(service: Service, id: string): Promise<void> {
  const result = await service.testNvidiaAccount(id)
  await notifyDaemonReload().catch(() => false)
  emitSuccess(result, () => {
    process.stdout.write(
...
async function runInfo(service: Service, id: string): Promise<void> {
  emitSuccess(await service.getNvidiaAccountInfo(id))
}

async function runRefreshModels(service: Service, id: string): Promise<void> {
  const result = await service.refreshNvidiaAccountModels(id)
  await notifyDaemonReload().catch(() => false)
  emitSuccess(result, () =>
...
async function runToggle(service: Service, id: string, enabled: boolean): Promise<void> {
  await service.toggleNvidiaAccount(id, enabled)
  await notifyDaemonReload().catch(() => false)
...
async function runRemove(service: Service, id: string): Promise<void> {
  await service.removeNvidiaAccount(id)
  await notifyDaemonReload().catch(() => false)
...
async function runReset(service: Service, id: string): Promise<void> {
  await service.resetNvidiaAccount(id)
  await notifyDaemonReload().catch(() => false)
...
async function runSetStatus(
  ...
  await service.setNvidiaAccountStatus(id, status as any, reason)
  await notifyDaemonReload().catch(() => false)
...
async function runChat(
  ...
  try {
    ...
    if (!response.ok || payload?.error) {
      throw new CliError(
        `NVIDIA chat failed: HTTP ${response.status} ${JSON.stringify(payload).slice(0, 800)}`,
        { code: ExitCode.GeneralError, errorCode: 'NVIDIA_CHAT_FAILED' }
      )
    }
    emitSuccess(payload, () => {
