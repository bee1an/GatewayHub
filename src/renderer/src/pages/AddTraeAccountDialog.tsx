...
export function AddTraeAccountDialog({
  open,
  onOpenChange,
  onImported
}: {
  ...
  async function handleRefreshToken() {
    ...
    setTokenMsg(null)
    try {
      await window.api.gateway.addTraeRefreshToken(refreshToken)
  ...
  async function handleJwtToken() {
    ...
    setTokenMsg(null)
    try {
      await window.api.gateway.addTraeJwtToken(jwtToken)
      setTokenMsg({ ok: true, text: t('addAccount.tokenValid') })
      setJwtToken('')
      onImported()
      onOpenChange(false)
  ...
  async function handleJson() {
    ...
    setJsonResult(null)
    try {
      const r = await window.api.gateway.importTraeJson(jsonText)
  ...
  async function handleScan() {
    ...
    try {
      const r = await window.api.gateway.scanTraeAccounts()
      setScanResult(r)
  ...
  async function handleImportSelected() {
    ...
    setDiscoverLoading(true)
    try {
      await window.api.gateway.importScannedTraeAccounts([...selectedIds])
      setScanResult(null)
      setSelectedIds(new Set())
      onImported()
      onOpenChange(false)
  ...
  return (
    <Modal open={open} onOpenChange={onOpenChange} title={t('addAccount.traeTitle')}>
      <TabGroup
        value={tab}
        ...
        items={[
          {
            ...
            content: (
              <div className="space-y-3">
                <p className="text-[12px] text-fog">{t('addAccount.traeRefreshHint')}</p>
                <textarea
                  value={refreshToken}
                  onChange={(e) => setRefreshToken(e.target.value)}
                  placeholder={t('addAccount.traeRefreshPlaceholder')}
          ...
          },
          {
            ...
            content: (
              <div className="space-y-3">
                <p className="text-[12px] text-fog">{t('addAccount.traeJwtHint')}</p>
                <textarea
                  value={jwtToken}
                  onChange={(e) => setJwtToken(e.target.value)}
                  placeholder={t('addAccount.traeJwtPlaceholder')}
                  className="input-base font-mono text-[12px] min-h-20 resize-y w-full"
                />
                <div className="flex items-center gap-3">
                  <Button
          ...
          },
          {
            ...
            content: (
              <div className="space-y-2">
                <p className="text-[12px] text-fog">{t('addAccount.traeDiscoverDesc')}</p>
                {!scanResult && (
                      ...
                      {scanResult.candidates.map((c) => (
                        ...
                        >
                          ...
                          <span className="tag text-[12px] !px-1 !py-0 shrink-0">
                            {c.existing ? t('gateway.exists') : c.sourceType || 'trae'}
                          </span>
                        </label>
                      ))}
                    </div>
          ...
          },
          {
            ...
            content: (
              <div className="space-y-2">
                <p className="text-[12px] text-fog">{t('addAccount.traeJsonHint')}</p>
                <textarea
                  className="w-full min-h-[120px] text-[12px] font-mono bg-pitch rounded-[var(--radius-md)] p-2 text-storm"
                  value={jsonText}
                  onChange={(e) => setJsonText(e.target.value)}
                  placeholder={t('addAccount.traeJsonPlaceholder')}
                />
                <div className="flex items-center gap-2">
                  <Button
                    variant="primary"
