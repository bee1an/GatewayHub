...
export function parseTraeWorkAuthInput(text: string): TraeWorkAccountConfig[] {
  ...
  try {
    ...
    const account = buildTraeWorkAccountFromInput({ jwtToken: trimmed })
    return account ? [account] : []
  }
  const items = extractCandidateObjects(parsed)
  const accounts: TraeWorkAccountConfig[] = []
  const seen = new Set<string>()
  for (const item of items) {
    const account = buildTraeWorkAccountFromInput(item)
...
export function buildTraeWorkAccountFromInput(input: any): TraeWorkAccountConfig | null {
  ...
  const id = userId
    ? `traework-user-${sha256Short(userId, 12)}`
    : refreshToken
      ? `traework-refresh-${sha256Short(refreshToken, 12)}`
      : `traework-jwt-${sha256Short(jwtToken || seed, 12)}`
  return {
    ...
    authType:
      ...
      (refreshToken ? 'traework-refresh-token' : 'traework-jwt'),
    deviceId: pickString(input.deviceId, input.device_id, input.deviceID) || undefined,
    machineId: pickString(input.machineId, input.machine_id) || undefined,
    devDeviceId: pickString(input.devDeviceId, input.dev_device_id) || undefined,
    deviceBrand: pickString(input.deviceBrand, input.device_brand) || undefined,
    osVersion: pickString(input.osVersion, input.os_version) || undefined,
