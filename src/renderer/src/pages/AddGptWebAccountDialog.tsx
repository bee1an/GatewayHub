...
export function AddGptWebAccountDialog({
  open,
  onOpenChange,
  onImported
}: {
  ...
  async function handleImport(): Promise<void> {
    ...
    setResult(null)
    try {
      const r = await window.api.gateway.importGptWebJson(jsonText)
      setResult({
        added: r.added,
        skipped: r.skipped,
  ...
  return (
    <Modal open={open} onOpenChange={onOpenChange} title={t('addAccount.gptWebTitle')}>
      <div className="space-y-3">
        <p className="text-[12px] text-fog">{t('addAccount.gptWebJsonHint')}</p>
        <textarea
          className="w-full min-h-[160px] text-[12px] font-mono bg-pitch rounded-[var(--radius-md)] p-2 text-storm"
          value={jsonText}
          onChange={(e) => setJsonText(e.target.value)}
          placeholder={t('addAccount.gptWebJsonPlaceholder')}
        />
