...
type GptWebAction = (typeof GPT_WEB_ACTIONS)[number]
type Service = Awaited<ReturnType<typeof ensureServiceInitialized>>

export function registerGptWebCommands(cli: CAC): void {
  cli
    .command(
      'gptWeb <action> [...args]',
      `GptWeb provider utilities (${GPT_WEB_ACTIONS.join('|')})`
    )
    .option('--reason <text>', 'Reason for set-status')
    .option('--model <model>', 'GptWeb model for chat smoke test', { default: 'auto' })
    ...
    .action(async (action: string, args: string[] = [], options: GptWebOptions) => {
      if (!GPT_WEB_ACTIONS.includes(action as GptWebAction)) {
        throw new CliError(
          `Unknown GptWeb action: ${action}. Expected one of ${GPT_WEB_ACTIONS.join(', ')}.`,
          { code: ExitCode.UsageError, errorCode: 'UNKNOWN_ACTION' }
        )
      }
      const service = await ensureServiceInitialized()
      switch (action as GptWebAction) {
        case 'list':
...
async function runList(service: Service): Promise<void> {
  ...
  const provider = status.providers.find((item) => item.providerType === 'gptWeb') as any
  const accounts: any[] = provider?.accounts ?? []
  emitSuccess(accounts, () => {
    if (isJsonMode()) return
    if (!accounts.length) {
      process.stdout.write(`${colors.dim('(no GptWeb accounts)')}\n`)
      return
    }
    printTable(accounts, [
...
async function runImportJson(service: Service, source: string | undefined): Promise<void> {
  ...
  if (!text) {
    throw new CliError('No GptWeb auth JSON content provided.', {
      code: ExitCode.UsageError,
      errorCode: 'MISSING_JSON'
    })
  }
  const result = await service.importGptWebAuthJson(text)
...
async function runTest(service: Service, id: string): Promise<void> {
  const result = await service.testGptWebAccount(id)
  await notifyDaemonReload().catch(() => false)
  emitSuccess(result, () => {
    process.stdout.write(
...
async function runInfo(service: Service, id: string): Promise<void> {
  emitSuccess(await service.getGptWebAccountInfo(id))
}

async function runRefreshModels(service: Service, id: string): Promise<void> {
  const result = await service.refreshGptWebAccountModels(id)
  await notifyDaemonReload().catch(() => false)
  emitSuccess(result, () =>
...
async function runToggle(service: Service, id: string, enabled: boolean): Promise<void> {
  await service.toggleGptWebAccount(id, enabled)
  await notifyDaemonReload().catch(() => false)
...
async function runRemove(service: Service, id: string): Promise<void> {
  await service.removeGptWebAccount(id)
  await notifyDaemonReload().catch(() => false)
...
async function runReset(service: Service, id: string): Promise<void> {
  await service.resetGptWebAccount(id)
  await notifyDaemonReload().catch(() => false)
...
async function runSetStatus(
  ...
  await service.setGptWebAccountStatus(id, status as any, reason)
  await notifyDaemonReload().catch(() => false)
...
async function runChat(
  ...
  try {
    ...
    if (!response.ok || payload?.error) {
      throw new CliError(
        `GptWeb chat failed: HTTP ${response.status} ${JSON.stringify(payload).slice(0, 800)}`,
        { code: ExitCode.GeneralError, errorCode: 'GPT_WEB_CHAT_FAILED' }
      )
    }
    emitSuccess(payload, () => {
